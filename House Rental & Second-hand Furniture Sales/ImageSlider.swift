...
class SliderGalleryController: UIViewController,UIScrollViewDelegate{
    ...
    func configurePlaceholder(){
        ...
        let font = UIFont.systemFont(ofSize: 17.0, weight: UIFont.Weight(rawValue: 1))
        let size = CGSize(width: self.scrollerViewWidth!, height: self.scrollerViewHeight!)
        placeholderImage = UIImage(named: "city")
    }
    ...
    func resetImageViewSource() {
        ...
        if self.currentIndex == 0 {
            self.leftImageView?.image = #imageLiteral(resourceName: "myApp")
            self.middleImageView?.image = #imageLiteral(resourceName: "loginitem")
            self.rightImageView?.image = #imageLiteral(resourceName: "loginbackground")
            self.leftImageView?.image = self.dataSource!.last!
            self.middleImageView?.image = self.dataSource!.first!
...
//                                               placeholder: placeholderImage)
            ...
            self.rightImageView?.image = self.dataSource![rightImageIndex]
//            self.rightImageView?.imageFromURL(self.dataSource![rightImageIndex],
//                                              placeholder: placeholderImage)
        }
            //当前显示的是最后一张图片
        else if self.currentIndex == (self.dataSource?.count)! - 1 {
            self.leftImageView?.image = self.dataSource![self.currentIndex-1]
            
            self.middleImageView?.image = self.dataSource!.last!
            
            self.rightImageView?.image = self.dataSource!.first!
...
//                                              placeholder: placeholderImage)
            ...
            self.leftImageView?.image = self.dataSource![self.currentIndex-1]
            self.middleImageView?.image = self.dataSource![self.currentIndex]
            self.rightImageView?.image = self.dataSource![self.currentIndex+1]
            
//            self.leftImageView?.imageFromURL(self.dataSource![self.currentIndex-1],
//                                             placeholder: placeholderImage)
//            self.middleImageView?.imageFromURL(self.dataSource![self.currentIndex],
//                                               placeholder: placeholderlwImage)
...
//class Imageslider: UIViewController, SliderGalleryControllerDelegate {
//
//    //获取屏幕宽度
...
//                                         action: #selector(Imageslider.handleTapAction(_:)))
